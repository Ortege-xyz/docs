...
## Wallet transactions (tbl_prod_sl_wallet_transactions)
...
| `amount` (conditional)            | The amount of STX locked for stacking, normalized to STX units (converted from microSTX to STX with a decimal precision of 6), depending on the stacking operation.             |
...
**Please note** This layer includes the volume in the native currency of each token and doesn't include pricing details. If you'd like it converted by the token's decimals and to have the realtime pricing information, that is available on the [Gold layer](gold.qmd).
