...
with sql.connect(server_hostname=hostname,
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute("SELECT * FROM external.db_btc.tbl_dev_sl_l2_stacks LIMIT 10")

        # Fetch the results into a Pandas DataFrame
        df = pd.DataFrame(cursor.fetchall(), columns=[desc[0] for desc in cursor.description])

# Create a table figure using Plotly's Graph Objects
fig = go.Figure(data=[go.Table(
    header=dict(values=list(df.columns),
                fill_color='paleturquoise',
                align='left'),
    cells=dict(values=[df[col].tolist() for col in df.columns],  # Convert each column's values to a list
               fill_color='lavender',
               align='left'))
])
...
# Load environment variables from .env file if not in CI/CD environment
load_dotenv()
...
# SQL query to fetch the count of name_register operations for the past 6 months
query = """
SELECT 
  DATE_TRUNC('month', TO_DATE(TO_TIMESTAMP(block_timestamp))) as month,
  COUNT(*) as register_count
FROM 
  external.db_stacks.tbl_dev_br_transactions
WHERE 
  contract_call.function_name = 'name-register'
  AND TO_DATE(TO_TIMESTAMP(block_timestamp)) >= CURRENT_DATE - INTERVAL '6 months'
GROUP BY 
  month
ORDER BY 
  month;
...
with sql.connect(server_hostname=db_host,
                 http_path=db_http,
                 access_token=db_token) as connection:
    df = pd.read_sql(query, connection)

# Ensure 'month' is a datetime type for proper plotting
df['month'] = pd.to_datetime(df['month'])

# Plotting the results with Plotly
fig = px.bar(df, x='month', y='register_count',
             title='Count of Name Register Operations Over the Past 6 Months',
             labels={'register_count': 'Register Count', 'month': 'Month'},
             text='register_count')
...
# SQL query
query = """
SELECT count(address) as address_count, decimals 
FROM external.db_stacks.vw_dev_sl_sip10_tokens 
GROUP BY decimals
"""
...
# Assuming the DataFrame has columns 'address_count' and 'decimals'
# Now, create a Plotly chart
fig = px.bar(df, x='decimals', y='address_count',
             labels={'decimals': 'Decimals', 'address_count': 'Address Count'},
             title='Count of Addresses Grouped by Decimals')

fig.update_layout(xaxis_title='Decimals', yaxis_title='Count of Addresses',
                  xaxis={'type': 'category'}, # Ensuring decimals are treated as discrete categories
...
FROM 
  external.db_stacks.vw_dev_sl_stx20
WHERE 
  date > '2023-12-23'
GROUP BY 
  stx_operation, 
...
# Adjust 'count' to 'hash_count' to match the AS clause in your SQL query
# Plotting the results with Plotly, changing to an area chart
fig = px.area(df, x='date', y='hash_count', color='stx_operation',
              title='Count of Operations by Date',
              labels={'hash_count': 'Count of Hash', 'date': 'Date', 'stx_operation': 'STX Operation'},
              line_group="stx_operation", # This ensures each operation type is treated as a separate line for the area fill
              markers=True)

fig.update_layout(xaxis_title='Date',
                  yaxis_title='Count of Transactions',
                  legend_title='STX Operation',
                  plot_bgcolor='white')
...
# SQL query to fetch last week's trading volumes for the top 5 tokens
query = """
WITH RankedTokens AS (
    SELECT
        tokenB,
        SUM(tokenBIn) / 1e8 AS total_volume,
        RANK() OVER (ORDER BY SUM(tokenBIn) DESC) as rank
    FROM external.db_stacks.vw_dev_sl_dapp_alex_swap
    WHERE block_timestamp >= CURRENT_DATE - INTERVAL '7 days'
    GROUP BY tokenB
)
SELECT tokenB, total_volume
FROM RankedTokens
WHERE rank <= 5;
"""
...
# Ensure you have results in the DataFrame
if not df.empty:
    # Plotting the results with Plotly as an area chart
    fig = px.area(df, x='tokenB', y='total_volume',
                  title='Last Week\'s Trading Volume for Top 5 Tokens',
                  labels={'total_volume': 'Total Volume', 'tokenB': 'Token'},
                  color='tokenB')  # Differentiating each token by color

    fig.update_layout(xaxis_title='Token',
                      yaxis_title='Total Trading Volume',
                      plot_bgcolor='white')

    # Adjust the x-axis to not display the token labels
    fig.update_xaxes(
    ...
    print("No data fetched. Please check your query or database connection details.")
